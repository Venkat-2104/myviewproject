...
def reviews(request,id):
    ...
    if request.method == 'POST':
        mreview = request.POST.get('_review')
...
def show_reviews(request,id):
    movie_inst = movie.objects.get(id = id)
    queryset = review.objects.filter(movie_name = movie_inst.name)
    return render(request,'show_reviews.html',{'page':'movie_reviews','review':queryset,'movie':movie_inst})